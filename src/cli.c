...
void command_line_interface_display(int argc, char** argv) {
    int cmd_version_argument_used = 0;

    int cmd_about_argument_used = 0; 

    cmd_version_argument_used = contains_argument(argc, argv, cmd_args_name[CMD_VERSION]) ||
                                contains_argument(argc, argv, cmd_args_name[CMD_VERSION_LONG]);

    cmd_about_argument_used = contains_argument(argc, argv, cmd_args_name[CMD_ABOUT]) || 
